...
export default function SectionPreviewCard({ href, label, hasContent, emptyMessage, children }: SectionPreviewCardProps) {
  return (
    <Link href={href} className="group rounded-[1.25rem]">
      <div className="h-72 overflow-hidden rounded-[1.25rem] border border-border bg-card">
        {hasContent ? (
          ...
          <div className="flex h-full items-center justify-center p-3 text-center text-xs text-muted-foreground">{emptyMessage}</div>
        )}
      </div>
      <p className="px-2 pt-2 text-base font-semibold text-foreground">{label}</p>
    </Link>
  )
}
